...
RUN CGO_ENABLED=1 GOOS=darwin go build -tags strictfipsruntime -a -o darwin-java-external-provider main.go
RUN CGO_ENABLED=1 GOOS=windows go build -tags strictfipsruntime -a -o windows-java-external-provider main.go
