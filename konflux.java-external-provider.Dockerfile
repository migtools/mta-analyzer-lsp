...
RUN CGO_ENABLED=0 GOOS=darwin go build -tags strictfipsruntime -a -o darwin-java-external-provider main.go
RUN CGO_ENABLED=0 GOOS=windows go build -tags strictfipsruntime -a -o windows-java-external-provider main.go
